...
auto evaluate_divison(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for /"};
    }
...
auto evaluate_divison_c(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for *"};
    }
...
auto evaluate_divison_rest(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for *"};
    }
...
{
    if (stack.size() != 1) {
        throw std::logic_error{"only one operand needed"};
    }
    auto const a = pop_top(stack);
    stack.push(sqrt(a));
}
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_divison(stack);
            } else if (each == "//") {
                evaluate_divison_c(stack);
            } else if (each == "%") {
                evaluate_divison_rest(stack);
            } else if (each == "**") {
